...
def meetsRequirements(ticker):
    ...
    if ((percentChange >= 0.05)and(volume >= 25000)):
...
for exchange in exchanges:
    ...
    for reportType in reportTypes:
        ...
        # see if the ticker meets requirements, if so increment hits
        hits = 0
        for ticker in tickers:
            try:
                if (meetsRequirements(ticker)):
                    hits = hits + 1
            except Exception:
                pass
